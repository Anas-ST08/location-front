...
import { fetchCarRental ,fetchLocations, createLocationAsync, } from "../redux/apiCall";
...
function EditeForm({ form, onChange, }) {
  ...
  useEffect(() => {
    if (form.immatricule) {
      const car = cars.find((car) => car._id === form.immatricule);
      ...
      <div className="row">
        ...
        <aside className="col-md-4 bg-light p-3">
          ...
          <div className="alert mt-3 text-center" style={{ fontSize: "50px" }}>
            {selectedCar?.pricePerDay || 0}.<span style={{ fontSize: "25px" }}>00 MAD</span>
          </div>
        </aside>
      </div>
    </div>
